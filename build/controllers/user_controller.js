...
const createActivationToken = (user) => {
    ...
    if (!process.env.ACTIVATION_SECRET) {
        throw new Error("Activation secret is not defined");
    }
...
exports.updateAccessTOken = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!session) {
            return next(new ErrorHandler_1.default("Please login to access this course", 400));
        }
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), "EX", 604800); // 604800 means 7 days acc to redis
        // res.status(200).json({
        //   status:"success",
        //   accessToken,
        // })
...
exports.updatePassword = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!oldPassword || !newPassword) {
            return next(new ErrorHandler_1.default("Please ente old and new Password", 400));
        }
        const user = await user_model_1.default.findById(req.user?._id).select("passwword");

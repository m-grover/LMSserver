...
exports.generateVideoUrl = exports.deleteCourse = exports.getAllUsers = exports.addReplyToReview = exports.addReview = exports.addAnswer = exports.addQuestion = exports.getCourseByUser = exports.getAllCourses = exports.getSingleCourse = exports.editCourse = exports.uploadCourse = void 0;
...
exports.getSingleCourse = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (isCacheExist) {
            ...
            const course = await course_model_1.default.findById(req.params.id).select("-courseData.videoUrl -courseData.questions -courseData.links");
            await redis_1.redis.set(courseId, JSON.stringify(course), "EX", 604800); //7 days expiry
            res.status(200).json({
                success: true,
                course,
            });
...
exports.getAllCourses = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const courses = await course_model_1.default.find().select("-courseData.videoUrl -courseData.questions -courseData.links");
        res.status(200).json({
            success: true,
            courses,
        });
...
exports.getCourseByUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!courseExists) {
            return next(new ErrorHandler_1.default("You are not eleigle to access this course", 404));
        }
    ...
    try {
        ...
        if (req.user?._id === question.user._id) {
            ...
            await notificationModel_1.default.create({
                ...
                message: `You have a new ques reply in ${courseContent.title}`,
            });
        }
        else {
            const data = {
            ...
            const html = await ejs_1.default.renderFile(path.join(__dirname, "../mails/question-reply.ejs"), data);
            try {
...
exports.addReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const userCourseList = req.user?.courses;
        const courseId = req.params.id;
        //check if courseId already exists in userCurseList based on _id
        const courseExists = userCourseList?.some((course) => course._id.toString() === courseId.toString());
        if (!courseExists) {
            return next(new ErrorHandler_1.default("You are not eligible to acess this course", 400));
        }
        const course = await course_model_1.default.findById(courseId);
        ...
        if (course) {
            course.ratings = avg / course?.reviews.length; // 9/2=4.5 ratings if one rating is 4 and second is 5
        }
        await course?.save();
        const notification = {
        ...
        return next(new ErrorHandler_1.default(error.meaage, 500));
    }
});
...
//redis is used to fetch data faster in case of heavy load on the server.
